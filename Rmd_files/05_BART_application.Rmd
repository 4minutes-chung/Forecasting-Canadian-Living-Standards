...
R_packages <- c("dbarts", "tidyverse", "lubridate", "knitr", "tibble", "here")
...
# 1. DATA as usual
```{r}
df_ml <- read.csv(here::here("data/raw/balanced_can_md.csv"))
...
write.csv(bart_short_all, here::here("outputs/forecasts/BART_all_variables_short.csv"), row.names = FALSE)
write.csv(bart_long_all,  here::here("outputs/forecasts/BART_all_variables_long.csv"),  row.names = FALSE)
```



# ARCHIVED: Old code (not executed)
```{r}
if (FALSE) {
  # Hard-coded comparison table from earlier CV runs
...
write.csv(
  bart_short_tbl,
  here::here("outputs/forecasts/BART_forecasts_short.csv"),
  row.names = FALSE
)

write.csv(
  bart_long_tbl,
  here::here("outputs/forecasts/BART_forecasts_long.csv"),
  row.names = FALSE
)

write.csv(
  comparison,
  here::here("outputs/forecasts/Model_Comparison_RMSE.csv"),
  row.names = FALSE
)
```
