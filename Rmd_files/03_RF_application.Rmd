...
R_packages <- c(
  ...
  "ranger", "tidyr", "tibble", "knitr", "here"
)
...
# 1. Data
```{r}
df_ml <- read_csv(here::here("data/raw/balanced_can_md.csv"), show_col_types = FALSE) %>%
  mutate(
    Date      = as.Date(Date),
...
write.csv(rf_short_all, here::here("outputs/forecasts/RF_all_variables_short.csv"), row.names = FALSE)
write.csv(rf_long_all,  here::here("outputs/forecasts/RF_all_variables_long.csv"),  row.names = FALSE)
```
